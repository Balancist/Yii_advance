...
class Generator extends \yii\gii\Generator
{
    ...
    public function requiredTemplates()
    {
        ...
        return ['base/BaseModel.php', 'model.php'];
    }
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            $files[] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/base/' . \yii\helpers\Inflector::camelize('base_'.$modelClassName). '.php',
                $this->render('base/BaseModel.php', $params)
            );
            // query :
            if ($queryClassName) {
    ...
    public function generateRules($table)
    {
        ...
        foreach ($table->columns as $column) {
            ...
            $comment[$column->name] = $column->comment;
        ...
        foreach ($types as $type => $columns) {
            if($type == 'required'){
                $rules[] = "[['" . implode("', '", $columns) . "'], '$type' , 'except' => 'getByParams']";
            }else {
            ...
            foreach ($uniqueIndexes as $uniqueColumns) {
                ...
                if (!$this->isColumnAutoIncremental($table, $uniqueColumns)) {
                    ...
                    if ($attributesCount === 1) {
                        ...
                        $rules[] = "[['$columnsList'], 'unique', 'targetAttribute' => ['$columnsList'], 'message' => Module::t('v1/app' , 'ترکیب فیلدهای {fields}  و {lastLabel} قبلا ثبت شده است.',['fields'=>'".implode(', ', $labels)."' ,'lastLabel'=> '$lastLabel'])]";
                    }
                }
            }
        ...
        foreach ($table->foreignKeys as $refs) {
            ...
            preg_match("/%(.*)%/m",$comment[$key],$matches);
            if(!empty($matches)){
                $rules[] = "[['$attributes'], \\whc\\education\\modules\\v1\\modules\\base_information\\components\\validators\\DomainValidator::className(),'skipOnError' => true, 'denote'=>'{$matches[1]}']";
            }else{
    ...
    public function generateString($string = '', $placeholders = [])
    {
        ...
        if ($this->enableI18N) {
            ...
            $str = "Module::t('" . $this->messageCategory . "', '" . $string . "'" . $ph . ")";
        } else {
